...
const KPI_CONFIG = [
    { key: 'Crecimiento_PIB_pct', label: 'PIB', unit: '%', color: 'yellow', icon: '📈', desc: 'Crecimiento anual' },
    { key: 'Presupuesto_Total_COP', label: 'Presupuesto', unit: 'B', color: 'blue', icon: '🏛️', desc: 'Billones COP' },
    { key: 'Gasto_Militar_pct_Presupuesto', label: 'Gasto Militar', unit: '%', color: 'red', icon: '⚔️', desc: 'Del presupuesto' },
    { key: 'Gasto_Educacion_pct_Presupuesto', label: 'Educación', unit: '%', color: 'blue', icon: '📚', desc: 'Del presupuesto' },
    { key: 'Inflacion_Anual_pct', label: 'Inflación', unit: '%', color: 'red', icon: '💸', desc: 'IPC anual' },
    { key: 'Desempleo_pct_Total', label: 'Desempleo', unit: '%', color: 'red', icon: '👥', desc: 'Tasa nacional' },
    { key: 'Inversion_Extranjera_pct_PIB', label: 'Inv. Extranjera', unit: '%', color: 'yellow', icon: '🌍', desc: '% del PIB' },
    { key: 'Salario_Minimo_COP', label: 'Salario Mínimo', unit: 'COP', color: 'blue', icon: '💰', desc: 'Mensual' },
    { key: 'Muertes_Conflicto', label: 'Conflicto', unit: '', color: 'red', icon: '🕊️', desc: 'Bajas en combate' },
];
...
export default function General() {
    ...
    useEffect(() => {
        fetch('http://127.0.0.1:8000/data')
            ...
            .then(json => {
                setData(json);
                setLoading(false);  // ✅ Bug corregido: llamadas separadas
            })
            .catch(() => {
                setError(true);
                setLoading(false);
    ...
    return (
        <div className="general-page">
            ...
            <div className="dashboard-container">
                ...
                {/* Panel del gráfico (próximo paso) */}
                <div className="main-chart-container glass-panel">
                    {activeKpi ? (
                        ...
                        <p style={{ color: 'var(--text-muted)' }}>👆 Selecciona un indicador de arriba para ver su evolución histórica</p>
                    )}
                </div>
